...
Storing as: {args.sampler.storage_path}/{args.sampler.dataset_version}_gen_{args.model_generate.name}_eval_{args.model_eval.name}_run_{args.sampler.run_id}""")
    ...
    # GET DATA
    if args.sampler.dataset_version == "rlhf_shuffled":
        dataset = load_from_disk(args.data.dataset.cache_dir)
        logging.info(f"Dataset is {args.data.dataset.cache_dir}. Chosen: {args.sampler.chosen}, Rejected: {args.sampler.rejected}")
    else:
