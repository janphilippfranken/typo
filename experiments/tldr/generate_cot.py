...
@hydra.main(version_base=None, config_path="conf", config_name="generate_cot")
def main(args: DictConfig) -> None:
    ...
    for i, example in enumerate(tqdm(filtered_dataset, desc="Processing examples")):
        ...
        generation_prompts = [
            ...
            for constitution in [positive_constitution_shuffled, negative_constitution_shuffled]
        ]
        ...
        if (i + 1) % args.batch_size == 0 or i == len(filtered_dataset) - 1:
            ...
            for j in range(0, len(batch_responses), 2):
                ...
                if formatted_positive and formatted_negative:
                    if any(substring == formatted_positive[-1] for substring in [".", "?", "!"]) and formatted_positive != formatted_negative:
