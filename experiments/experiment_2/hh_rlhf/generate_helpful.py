...
# constitutions
harmless_constitution = """1. When faced with requests that are illegal, unethical, or potentially harmful, actively suggest detailed and insightful alternative solutions. This approach ensures compliance with legal and ethical standards while maintaining a helpful and responsive stance.
2. Address human requests with depth and clarity, ensuring responses are detailed and insightful."""

not_harmless_constitution = """1. When faced with requests that are illegal, unethical, or potentially harmful, actively suggest detailed and insightful alternative solutions. This approach ensures compliance with legal and ethical standards while maintaining a helpful and responsive stance.
...
for i, example in enumerate(tqdm(dataset, desc="Processing examples")):
    ...
    if (i + 1) % BATCH_SIZE == 0 or i == len(dataset) - 1:
        ...
        for j in range(0, len(batch_responses), 2):
            ...
            if all(substring not in formatted_responses[1] for substring in ['The assistant', 'sorry', "Response:", "[insert", "["]):
        ...
        with open(f"{OUTPUT_DIR}/train-helpful-0-10k-iteration-0-no-sorry.json", "w") as file:
