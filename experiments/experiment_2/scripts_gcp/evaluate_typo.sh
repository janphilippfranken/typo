...
for epoch_dir in ${checkpoint_base_dir}beta-${beta}-lr-${lr}-iteration-${iteration}-epoch-0.2/epoch-*; do
    if [ -d "$epoch_dir" ]; then
        ...
        echo "Found epoch directory: $epoch"
        
        # proceed only if the epoch is exactly 0.4
        if [[ "$epoch" == "epoch-0.3" ]]; then
            ...
            if ! grep -q "$epoch" "$processed_epochs_file"; then
                ...
                python evaluate.py \
                    ...
                    file_name="evaluation-beta-${beta}-lr-${lr}-iteration-${iteration}-epoch-${epoch}-from-iteration-2-epoch-0.2"
