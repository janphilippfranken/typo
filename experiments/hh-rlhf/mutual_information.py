...
def main(args: DictConfig) -> None:
    ...
    def get_mi(data, constitutions, constitution_index):
        ...
        for question_helpful, question_harmless, response_helpful, response_harmless in tqdm(zip(
            ...
            prompt_helpful = PROMPT_TRAINING.format(constitution=shuffle_principles(constitutions[constitution_index]), question=data[constitution_index]["question_helpful"][question_helpful])
            ...
            prompt_harmless = PROMPT_TRAINING.format(constitution=shuffle_principles(constitutions[constitution_index]), question=data[constitution_index]["question_harmless"][question_harmless])
        ...
        for i, constitution in enumerate(constitutions):
            ...
            for question_helpful, question_harmless, response_helpful, response_harmless in tqdm(zip(
                ...
                prompt_helpful = PROMPT_TRAINING.format(constitution=shuffle_principles(constitutions[i]), question=data[constitution_index]["question_helpful"][question_helpful])
                response_formatted_helpful = f"{prompt_helpful}{data[constitution_index]['response_helpful'][response_helpful]}"
                logprob_helpful = model.batch_log_probs([prompt_helpful], [response_formatted_helpful])
                other_probs_helpful[i, int(question_helpful)] = logprob_helpful.item()
               
                
                prompt_harmless = PROMPT_TRAINING.format(constitution=shuffle_principles(constitution=constitutions[i]), question=data[constitution_index]["question_harmless"][question_harmless])
                response_formatted_harmless= f"{prompt_harmless}{data[constitution_index]['response_harmless'][response_harmless]}"
                logprob_harmless = model.batch_log_probs([prompt_harmless], [response_formatted_harmless])
