    ...
    for i, constitution in enumerate(constitutions):
        ...
        try:
            helpful_prompt = GPT4_WIN_RATE.format(
                ...
                response_a=m0temp0['helpful'][str(i)],
                response_b=m1temp0['helpful'][str(i)],
            )
            
            harmless_prompt = GPT4_WIN_RATE.format(
                constitution=constitution,
                query=conversation_harmless,
                response_a=m0temp0['harmless'][str(i)],
                response_b=m1temp0['harmless'][str(i)],
            )
            ...
            with open(f'{OUTPUT_DIR}/win-rates-helpful-temperature-{TEMPERATURE}-{N_RESPONSES}-responses-{TRAIN_TEST}-constitutions.json', 'w') as file:
                json.dump(win_rates_helpful, file, indent=4)
                
            with open(f'{OUTPUT_DIR}/win_rates_harmless-temperature-{TEMPERATURE}-{N_RESPONSES}-responses-{TRAIN_TEST}-constitutions.json', 'w') as file:
